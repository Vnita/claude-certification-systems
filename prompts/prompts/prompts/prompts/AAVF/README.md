...
|--------|---------------|----------------------|
| 1. Construct Validity | Does the test measure what it claims? | Style bias as construct contamination — Claude rewards well-structured prose even when structure isn't the competency |
| 2. Reliability | Consistent scores across instances? | Prompt sensitivity, context position effects, temperature variance — sources of variance human raters don't have |
| 3. Fairness | Equitable scores across groups? | Inherited training bias — embedded in weights, cannot be directly corrected, only detected and designed around |
| 4. Consequential Validity | Do decisions predict job performance? | No baseline data at launch — evidence collection plan must be designed before deployment, not after |
| 5. Temporal Validity ⭐ | — | **Original contribution.** No equivalent in published literature. See `/AAVF/pillar-5-temporal-validity.md` |
...
Meeting this standard does not mean the certification is perfect. It 
means it is:
...
- CCEE Component 3 (Quality Auditor) implements Pillars 1, 2, and 3 
  at the item level
- CCEE Component 4 (Capability Checker) implements Pillar 5 
  (Temporal Validity) at the item bank level
- Pillar 4 (Consequential Validity) requires post-launch outcome data — 
  collection plan must be designed before first cohort graduates
